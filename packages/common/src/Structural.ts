 ...
 *
 * Structural keys are JSON-like values with `undefined`, `NaN`, and
 * positive/negative infinity also supported, or `Uint8Array`.
 *
 ...
 * ```
 */
export interface StructuralMap<K, V>
  extends Iterable<readonly [Structural<K>, V]> {
...
const createUnsupportedStructuralKeyError = (value: unknown): Error =>
  new Error(
    `Structural keys must be JSON-like values, undefined, or Uint8Array; received ${formatUnsupportedStructuralKeyType(value)}.`,
  );
