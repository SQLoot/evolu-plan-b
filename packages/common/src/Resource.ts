...
export interface SharedResourceByKeyOptions<K>
    ...
    return ok({
                  ...
                  createSharedResource(create(key as K), {
                    ...
                    onDisposed: () => {
                      ...
                      void sharedResourceByKeyRun(
                        mutexByKey.withLock(toMutexKey(key), async () => {
                          if (
                            sharedResource &&
      ...
      snapshot: () => {
        ...
        for (const [key, sharedResource] of sharedResourcesByKey.entries()) {
          ...
          mutexSnapshotsByKey.set(key, mutexByKey.snapshot(toMutexKey(key)));
        }
...
export interface SharedResourceByKeyWithClaimsOptions<K, T extends Resource>
  extends Pick<SharedResourceOptions, "idleDisposeAfter"> {
            ...
            sharedResourceClaimsRun(async (run) => {
              ...
              for (const resourceKey of resourceKeys) {
                const added = await run(
                  mutexByKey.withLock(toMutexKey(resourceKey), async (run) => {
