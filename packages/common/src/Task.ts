 ...
 */
export type Concurrency = PositiveInt;
...
const createRunnerInternal =
	...
	(
...

		{
			...
			run.onAbort = (callback: (reason: unknown) => void) => {
			...
			run.complete = (
				taskResult: Result<unknown, unknown>,
...
const isAbortable = AbortMask.orThrow(0 as any);
 ...
 *
 * Similar to
 * {@link https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/withResolvers | withResolvers},
...
export interface ForEachAbortError
	extends InferType<typeof ForEachAbortError> {}
...
export interface ForEachSettledAbortError
	extends InferType<typeof ForEachSettledAbortError> {}
...
function concurrent<T, E>(
	...
	const tasks = ensureArray(tasksIterable);
	const { length } = tasks;
	...
	return async (run) => {
		...
		const stopSignal = stopOn ? withResolvers<void>() : null;
		...
		const workers = createArray(workerCount, () => run.daemon(worker));
