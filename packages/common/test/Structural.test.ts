...
describe("StructuralMap", () => {
  ...
  test("treats sparse arrays like explicit undefined entries", () => {
    const map = createStructuralMap<readonly [undefined], string>();

    map.set(Array(1) as never, "value");

    expect(map.get([undefined])).toBe("value");
  });
  ...
  test("rejects keys outside JSON-like values and Uint8Array", () => {
    ...
    expect(() => map.set(new Example() as never, "value")).toThrow(
      "Structural keys must be JSON-like values, undefined, or Uint8Array; received Example.",
    );
  });

  test("supports top-level undefined keys", () => {
    const map = createStructuralMap<string | undefined, string>();
