...
const createSymbolWithoutDisposableStatics = (): SymbolConstructor => {
  ...
  for (const key of Reflect.ownKeys(nativeSymbol)) {
    ...
    if (Object.hasOwn(wrappedSymbol, key)) continue;
...
describe("DisposableStack behavior", () => {
  ...
  test("throws on invalid use/defer/adopt input and move on disposed stack", () => {
    ...
    expect(() =>
      ...
      /Cannot call DisposableStack\.prototype\.use on an already-disposed DisposableStack|DisposableStack\.prototype\.use requires that \|this\| be a pending DisposableStack object/,
    );
    expect(() => stack.defer(() => undefined)).toThrow(
      /Cannot call DisposableStack\.prototype\.defer on an already-disposed DisposableStack|DisposableStack\.prototype\.defer requires that \|this\| be a pending DisposableStack object/,
    );
    expect(() => stack.adopt("x", () => undefined)).toThrow(
      /Cannot call DisposableStack\.prototype\.adopt on an already-disposed DisposableStack|DisposableStack\.prototype\.adopt requires that \|this\| be a pending DisposableStack object/,
    );
    expect(() => stack.move()).toThrow(
      /Cannot call DisposableStack\.prototype\.move on an already-disposed DisposableStack|DisposableStack\.prototype\.move requires that \|this\| be a pending DisposableStack object/,
    );
  });
