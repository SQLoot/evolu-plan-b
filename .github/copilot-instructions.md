...
# Evolu Plan B project guidelines

## Fork constraints

- Use **Bun** only (`bun install`, `bun run ...`)
- Do not add or use `pnpm`, `npm`, or `yarn`
...
TypeScript language service may show stale cross-package errors after package changes. If types look inconsistent, restart the TypeScript server in the editor.

## Architecture

Monorepo with Bun workspaces and Turborepo. All packages depend on `@evolu/common`:
